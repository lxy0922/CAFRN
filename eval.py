...
parser.add_argument('--input_dir', type=str, default='./Input', help='Location of testing images')
parser.add_argument('--output', default='', help='Location to save output images')
parser.add_argument('--test_dataset', type=str, default='WHU_x2')
parser.add_argument('--model_type', type=str, default='CAFRN')
parser.add_argument('--residual', type=bool, default=False)
parser.add_argument('--model', default='models/CAFRN_x2.pth', help='sr pretrained base model')
...
test_set = get_eval_set(os.path.join(opt.input_dir), opt.upscale_factor)
testing_data_loader = DataLoader(dataset=test_set, num_workers=opt.threads, batch_size=opt.testBatchSize, shuffle=False)

print('===> Building model')
model = CAFRN(in_channels=3, out_channels=3, num_features=64, num_steps=7, reduction=16, upscale_factor=opt.upscale_factor, act_type = 'prelu', norm_type = None) 

if cuda:
    model = torch.nn.DataParallel(model, device_ids=gpus_list)

model = torch.load(opt.model)["model"]
print('Pre-trained SR model is loaded:', opt.model)
